...
(function (jQuery) {
    ...
    jQuery.fn.jqFancyTransitions = jQuery.fn.jqfancytransitions = function (options) {
        init = function (el) {
            ...
            jQuery('#' + el.id).css({'background-image': 'url(' + img[el.id][0] + ')', 'width': params.width, 'height': params.height, 'position': 'relative', 'background-position': 'top left', });
            jQuery('#' + el.id).append("<div class='ft-title' id='ft-title-" + el.id + "'>" + titles[el.id][0] + "</div>");
            ...
            for (j = 1; j < params.strips + 1; j++) {
                ...
                jQuery("#ft-" + el.id + j).css({'background-position': -stripLeft + 'px top', 'left': stripLeft});
            ...
            jQuery('.ft-' + el.id).mouseover(function () {
                ...
                jQuery('#ft-prev-' + el.id).css({'opacity': .5, });
                jQuery('#ft-next-' + el.id).css({'opacity': .5, });
            });
            jQuery('.ft-' + el.id).mouseout(function () {
                opts[el.id].pause = false;
                jQuery('#ft-prev-' + el.id).css({'opacity': .1, });
                jQuery('#ft-next-' + el.id).css({'opacity': .1, });
            });
            ...
            jQuery('.ft-prev').mouseover(function () {
                jQuery('#ft-prev-' + el.id).css({'opacity': .5, });
                jQuery('#ft-next-' + el.id).css({'opacity': .5, });
            });
            jQuery('.ft-next').mouseover(function () {
                jQuery('#ft-prev-' + el.id).css({'opacity': .5, });
                jQuery('#ft-next-' + el.id).css({'opacity': .5, });
            });
            jQuery('.ft-prev').mouseout(function () {
                jQuery('#ft-prev-' + el.id).css({'opacity': .1, });
                jQuery('#ft-next-' + el.id).css({'opacity': .1, });
            });
            jQuery('.ft-next').mouseout(function () {
                jQuery('#ft-prev-' + el.id).css({'opacity': .1, });
                jQuery('#ft-next-' + el.id).css({'opacity': .1, });
            });
        ...
        jQuery.transition = function (el, direction) {
            ...
            stripInt[el.id] = setInterval(function () {
                jQuery.strips(order[el.id][inc[el.id]], el)
        ...
        jQuery.navigation = function (el) {
            jQuery('#' + el.id).append("<a href='#' id='ft-prev-" + el.id + "' class='ft-prev'>&nbsp;</a>");
            jQuery('#' + el.id).append("<a href='#' id='ft-next-" + el.id + "' class='ft-next'>&nbsp;</a>");
            jQuery('#ft-prev-' + el.id).css({'position': 'absolute', 'top': params.height / 2 - 15, 'left': 5, 'z-index': 1001, }).click(function (e) {
                e.preventDefault();
            ...
            jQuery('#ft-next-' + el.id).css({'position': 'absolute', 'top': params.height / 2 - 15, 'right': 5, 'z-index': 1001, }).click(function (e) {
                e.preventDefault();
                jQuery.transition(el);
                clearInterval(imgInt[el.id]);
                imgInt[el.id] = setInterval(function () {
            ...
            jQuery('.ft-button-' + el.id).css({'padding': 5});
    ...
    jQuery.fn.jqFancyTransitions.defaults = {width: 1300, height: 400, strips: 20, delay: 5000, stripDelay: 50, titleOpacity: 1, titleSpeed: 1000, position: 'alternate', direction: 'fountainAlternate', effect: 'zipper', navigation: false, links: false};
})(jQuery);
jQuery(window).ready(function () {
     var  height1   = jQuery('#slide').css('width',"100%");
    jQuery('#slide').jqFancyTransitions({  width: height1, height: 410, strips: 19, position: 'alternate', direction: 'fountainAlternate', effect: 'zipper', navigation: true, links: false    });
});
